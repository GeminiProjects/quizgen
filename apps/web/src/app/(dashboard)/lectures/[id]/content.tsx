...
export default function LectureDetailContent({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 标签页 */}
      <Tabs className="space-y-6" defaultValue="quiz">
        <TabsList className="grid w-full grid-cols-3">
          <TabsTrigger value="quiz">测验题目</TabsTrigger>
          <TabsTrigger value="control">演讲控制</TabsTrigger>
          <TabsTrigger value="details">详细信息</TabsTrigger>
        </TabsList>

        {/* 测验题目 */}
        <TabsContent className="space-y-4" value="quiz">
          {lecture.quizItems && lecture.quizItems.length > 0 ? (
            <>
              <div className="grid gap-3">
                {lecture.quizItems.map((quiz: QuizItemWithDate) => (
                  <Card key={quiz.id}>
                    <CardHeader className="pb-3">
                      ...
                      <CardDescription className="text-xs">
                        创建于 {formatRelativeTime(quiz.created_at)}
                      </CardDescription>
                    </CardHeader>
              ...
              {/* 加载更多按钮 */}
              {lecture.quizItems.length >= 10 && (
                <div className="flex justify-center pt-4">
                  <Button variant="outline">查看全部题目</Button>
                </div>
              )}
            </>
          ) : (
